...
/** Generates a scenario by actually dealing a random-length sequence
 * from a fresh shoe (reusing the shared shoe engine, per CLAUDE.md
 * rule #2) rather than synthesizing arbitrary running-count/decks
 * pairs out of thin air — keeps scenarios in a realistic range. */
function generateScenario(systemId: string, deckCount: number): Scenario {
...
export default function TrueCountDrillPage() {
  ...
  const [scenario, setScenario] = useState<Scenario>(() => generateScenario("hi-lo", 6));
  ...
  function newScenario(nextSystemId: string = systemId, nextDeckCount: number = deckCount) {
    setScenario(generateScenario(nextSystemId, nextDeckCount));
    setGuess("");
    setFeedback(null);
  }
      ...
      <section className="felt-panel mt-6 p-4">
        ...
        <div className="mt-6 flex flex-wrap items-center justify-center gap-2">
          ...
          <input
            ...
            className="felt-panel w-24 rounded-card bg-felt-900 px-2 py-1 text-sm text-ink"
          />
          <button
            type="button"
            onClick={checkGuess}
